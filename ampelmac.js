...
changeLights = (status) => {
  if (status === "SUCCESS") {
          cmd = spawn( './clewarecontrol.dms',  ["-c" , "1" , "-as" , "2" , "1", "-as", "0", "0"]);
          ...
          cmd = spawn( './clewarecontrol.dms',  ["-c" , "1" , "-as" , "0" , "1", "-as" , "2" , "0"  ]);
          cmd.stdout.on( 'data', data => console.log( `stdout: ${data}` ) );
          cmd.stderr.on( 'data', data => console.log( `stderr: ${data}` ) );
          cmd.on( 'error', code => console.log( `child process exited with code ${code}` ) );
         }
...
//conslog(req.url)
  ...
  if (light === "break" && onof === "update") {
    cmd = spawn( './clewarecontrol.dms',  ["-c" , "1" , "-as" , "2" , "0", "-as", "0", "0" , "-as" , "1" , "0"])
    checkBuildState(changeLights)   
  } else  if (light === "update" && onof === "break") {
    cmd = spawn( './clewarecontrol.dms',  ["-c" , "1" , "-as" , "2" , "0", "-as", "0", "0" , "-as" , "1" , "0"])
    checkBuildState(changeLights) 
  }
   else{
